...
** Description:   Função para desenhar e mostrar as opçoes de contexto
***************************************************************************************/
void drawSubmenu(int index, std::vector<Option> &options, const char *title) { // <-- pour bien la voir
    drawStatusBar();
     ...
     */
    // Zone de contenu centrale : tout entre le titre et le bord bas
    int contentTop = 30;
    int contentBottom = tftHeight - 5;
    int middle = contentTop + (contentBottom - contentTop) / 2;

    // On efface toute la zone de contenu (fini les bandes prev/next résiduelles)
    tft.fillRect(6, contentTop, tftWidth - 12, contentBottom, bruceConfig.bgColor);

    // Une seule option, centrée, police bitmap classique (pas de SMOOTH_FONT -> pas d'anti-aliasing)
    String label = options[index].label;
    bool needsSplit = label.length() > 10 || label.indexOf(' ') >= 0 || label.indexOf('-') >= 0;

    tft.setTextColor(options[index].enabled ? bruceConfig.priColor : TFT_DARKGREY, bruceConfig.bgColor);

    if (!needsSplit) {
        int selectedTextSize = label.length() <= tftWidth / (LW * FG) - 1 ? FG : FM;
        tft.setTextSize(selectedTextSize);
        tft.drawCentreString(label, tftWidth / 2, middle - selectedTextSize * LH / 2, 1);
    } else {
        // Cherche le meilleur point de coupure (espace/tiret le plus proche du milieu du texte)
        int splitPos = -1;
        int center = label.length() / 2;
        for (int i = 0; i < (int)label.length(); i++) {
            char c = label[i];
            if (c == ' ' || c == '-' || c == '_') {
                if (splitPos == -1 || abs(i - center) < abs(splitPos - center)) splitPos = i;
            }
        }

        String line1, line2;
        if (splitPos == -1) {
            // Pas de séparateur trouvé : coupe au milieu du mot
            line1 = label.substring(0, center);
            line2 = label.substring(center);
        } else {
            line1 = label.substring(0, splitPos);
            line2 = label.substring(splitPos + 1); // on saute le séparateur lui-même
        }

        int selectedTextSize = FM; // taille réduite car 2 lignes à caser
        tft.setTextSize(selectedTextSize);
        tft.drawCentreString(line1, tftWidth / 2, middle - selectedTextSize * LH, 1);
        tft.drawCentreString(line2, tftWidth / 2, middle, 1);
    }
    // tft.drawFastHLine(
    ...
    // Barre de progression latérale : indique la position dans la liste (gardée, utile sans les 2 autres
    // lignes)
    int menuSize = options.size();
